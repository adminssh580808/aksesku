...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/adminssh580808/aksesku/main/ipakses > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/adminssh580808/aksesku/main/ipakses | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/adminssh580808/aksesku/main/ipakses | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/adminssh580808/ssdi/main/allow | grep $MYIP | awk '{print $3}')
fi
clear
echo -e "\e[36m╒════════════════════════════════════════════╕\033[0m"
...
serverV=$( curl -sS https://raw.githubusercontent.com/adminssh580808/ssdi/main/versi)
ISP=$(curl -sS https://ipinfo.io/org | cut -d " " -f 2-100)
echo -e " ISP NAME       :  $ISP"
if [ "$cekup" = "day" ]; then
...
echo -e "\e[36m╘════════════════════════════════════════════╛\033[0m
 ...
 [\033[1;36m09\033[0m]  ADD TROJAN ACCOUNT
...
echo -e "\e[36m╘════════════════════════════════════════════╛\033[0m
 ...
 [\033[1;36m15\033[0m]  RENEW CERT SSL 
...
curl -sS https://raw.githubusercontent.com/adminssh580808/ssdi/main/clgshow
echo -e "
"
fi
echo -e "\e[36m╘═════════════════════════════════════════════╛\033[0m"
...
if [[ $(cat /opt/.ver) = $serverV ]] > /dev/null 2>&1; then
    ...
    if [[ $x -eq 69 ]]; then
       wget -q -O /usr/bin/update-script "https://raw.githubusercontent.com/adminssh580808/ssdi/main/update-script.sh" && chmod +x /usr/bin/update-script
